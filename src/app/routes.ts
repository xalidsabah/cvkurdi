...
function buildRouteTree(dir: string, basePath = ''): Tree {
	...
	for (const file of files) {
		...
		} else if (file === 'page.jsx' || file === 'route.js') {
			node.hasPage = true;
    }
	}
...
function generateRoutes(node: Tree): RouteConfigEntry[] {
	...
	if (node.hasPage) {
		// Check if it's an API route or page route
		const isApiRoute = node.path.includes('/api/');
		let componentPath;

		if (isApiRoute) {
			// For API routes, use route.js
			componentPath = node.path === '' ? `./${node.path}route.js` : `./${node.path}/route.js`;
		} else {
			// For page routes, use page.jsx
			componentPath = node.path === '' ? `./${node.path}page.jsx` : `./${node.path}/page.jsx`;
		}
